...
Section Obs.
  ...
  Lemma obs_value {d n ts tu} :
    F.Value ts → E.Value tu → Obs d n ts tu.
  Proof.
    intros vs vu.
    destruct d; simpl; intros _;
    eauto using F.values_terminate, E.values_terminate.
  Qed.
  ...
  Proof.
    ...
    eauto using F.TermHor_lt, E.TermHor_lt.
  Qed.
...
Section ClosedLR.
  ...
  Lemma envrel_implies_WtSub_equi {d W Γ γs γu} :
    envrel d W Γ γs γu → E.WtSub (fxToIsCtx Γ) E.empty γu.
  Proof.
    intros er i τ vu_τ.
  ...
  Proof.
    ...
    unfold termrel, termrel'.
    intros Cs Cu ecs ecu cr.
    refine (obs_antired _ _ sge _); eauto using F.evaln_ctx, E.evaln_ctx.
    apply tr; auto.
  ...
  Proof.
    intros tr term ineq.
    specialize (tr F.phole E.phole I I contrel_triv).
...
Section OpenLR.
  ...
  Proof.
    ...
    assert (er : envrel dir_lt w pempty (idm F.Tm) (idm E.Tm)) by apply envrel_triv.
    pose proof (lr w le_w (idm F.Tm) (idm E.Tm) er) as tr.
    rewrite -> ?ap_id in tr.
  ...
  Proof.
    ...
    set (w := n).
    assert (le_w : lev w ≤ n) by (unfold lev, w; lia).
    assert (er : envrel dir_gt w pempty (idm F.Tm) (idm E.Tm)) by apply envrel_triv.
    (* assert (er : envrel dir_lt w pempty (idm F.Tm) (idm E.Tm)) by apply envrel_triv. *)
    pose proof (lr w le_w (idm F.Tm) (idm E.Tm) er) as tr.
    (* pose proof (lr w le_w (idm F.Tm) (idm E.Tm) er) as tr. *)
    rewrite -> ?ap_id in tr.
...
Section TermRelZero.
  ...
  Lemma termrel₀_ectx {d dfc w τ₁ τ₂ ts Cs tu Cu} (eCs : F.ECtx Cs) (eCu : E.ECtx Cu) :
    termreli₀ d dfc w τ₁ ts tu →
    (∀ vs vu, valrel d w τ₁ vs vu → termreli₀ d dfc w τ₂ (F.pctx_app vs Cs) (E.pctx_app vu Cu)) →
    termreli₀ d dfc w τ₂ (F.pctx_app ts Cs) (E.pctx_app tu Cu).
  Proof.
    intros trtm trcont.
    ...
    - right.
      ...
      eauto using F.ectx_cat, E.ectx_cat.
  Qed.
  ...
  Lemma termrel₀_ectx'' {d w' w τ₁ τ₂ ts Cs tu Cu} (eCs : F.ECtx Cs) (eCu : E.ECtx Cu) :
    termrel₀ d w' τ₁ ts tu →
    (∀ vs vu, valrel d w' τ₁ vs vu → termrel₀ d w τ₂ (F.pctx_app vs Cs) (E.pctx_app vu Cu)) →
    w ≤ w' →
  ...
  Proof.
    ...
    - right.
      ...
      rewrite <- F.pctx_cat_app.
      rewrite <- E.pctx_cat_app.
      eauto using F.ectx_cat, E.ectx_cat, obs_mono.
  Qed.
  ...
  Lemma termreli₀_ectx {d dfc w τ₁ τ₂ ts Cs tu Cu} (eCs : F.ECtx Cs) (eCu : E.ECtx Cu) :
    termrel₀ d (lateri dfc w) τ₁ ts tu →
    lev w ≥ dfc →
    (∀ vs vu, valrel d (lateri dfc w) τ₁ vs vu → termreli₀ d dfc w τ₂ (F.pctx_app vs Cs) (E.pctx_app vu Cu)) →
    termreli₀ d dfc w τ₂ (F.pctx_app ts Cs) (E.pctx_app tu Cu).
  Proof.
...
Section TermRelZeroNoDiv.
  ...
  Lemma termrelnd₀_ectx {d w w' τ₁ τ₂ ts Cs tu Cu} (eCs : F.ECtx Cs) (eCu : E.ECtx Cu) :
    termrelnd₀ d w τ₁ ts tu →
    (∀ vs vu, valrel d w τ₁ vs vu → termrelnd₀ d w' τ₂ (F.pctx_app vs Cs) (E.pctx_app vu Cu)) →
    termrelnd₀ d w' τ₂ (F.pctx_app ts Cs) (E.pctx_app tu Cu).
  Proof.
    intros tr cr.
    destruct d.
    - intros vs vvs es.
  ...
  Lemma termrelnd₀_ectx_sub {d w τ₁ τ₂ ts vu tub} Cs (eCs : F.ECtx Cs) {vvu : E.Value vu} :
...
Ltac crushLRMatch :=
  match goal with
    ...
    | [ |- termrel _ _ _ (F.abs _ _) (E.abs _ _) ] => apply valrel_in_termrel
    | [ |- termrel _ _ _ F.unit E.unit ] => apply valrel_in_termrel
    | [ |- termrel _ _ _ F.false E.false ] => apply valrel_in_termrel
    | [ |- termrel _ _ _ F.true E.true ] => apply valrel_in_termrel
