...
Definition prod_rel (R₁ R₂ : F.Tm → I.Tm → Prop) : F.Tm → I.Tm → Prop :=
  fun ts tu =>
    match ts , tu with
      | F.pair ts₁ ts₂ , I.pair tu₁ tu₂ => R₁ ts₁ tu₁ ∧ R₂ ts₂ tu₂
      | _              , _              => False
    end.
...
Section LogicalRelation.
  ...
  Definition Obs (w : World) (ts : F.Tm) (tu : I.Tm) :=
    match d with
      | dir_lt => Observe (lev w) (F.TermHor ts) → I.Terminating tu
  ...
  Definition termrel' (w : World) (vr' : ∀ w' : World, w' ≤ w → PTRel) : PTRel :=
    fun τ ts tu => ∀ Cs Cu,
        F.ECtx Cs → I.ECtx Cu → contrel' w vr' τ Cs Cu →
        Obs w (F.pctx_app ts Cs) (I.pctx_app tu Cu).
  ...
  Definition valrel' (w : World) (ind : ∀ w' : World, w' < w → PTRel) : PTRel :=
    fun τ ts tu =>
      ...
      let vrbool : F.Tm → I.Tm → Prop := fun ts tu => (ts = F.true ∧ tu = I.true) ∨ (ts = F.false ∧ tu = I.false) in
      let vrprod : PTy → PTy → F.Tm → I.Tm → Prop :=
          fun τ₁ τ₂ =>
        ...
        | pEmulDV n p τ' => match n with
                           ...
                           | S n' => (ts = unkUVal n ∧ p = imprecise) ∨
                                    ...
                                    match τ' with
                                     ...
                                     | I.tbool => is_inl ts ts' ∧ vrbool ts' tu
                                     | I.tprod τ₁ τ₂ => is_inl ts ts' ∧ vrprod (pEmulDV n' p τ₁) (pEmulDV n' p τ₂) ts' tu
...
Section TermRelZeroNoDiv.
  Definition termrelnd₀ d w τ ts tu :=
    ...
    | dir_lt => forall vs, F.Value vs -> clos_refl_trans_1n F.Tm F.eval ts vs ->
                     ∃ vu, I.Value vu /\ tu -->* vu ∧ valrel d w τ vs vu
    | dir_gt => forall vu, I.Value vu -> tu -->* vu ->
