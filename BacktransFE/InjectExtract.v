...
Definition extract_works_prop (n : nat) (w : World) (d : Direction) (p : Prec) (vs : F.Tm) (vu : E.Tm) (τ : F.Ty) : Prop :=
  ...
  (*        ∧ valrel d w (embed τ) vs' vu) *)
  (* ∨ Obs d w (F.pctx_app (F.app (extract n τ) vs) Cs) (E.pctx_app vu Cu). *)
...
Proof.
  ...
  - cbn.
    ...
    apply valrel_inUnit; split;
      [F.stlcCanForm | E.stlcCanForm]; trivial.
    eauto using E.tyeq_refl.
Qed.
...
Proof.
  ...
  - apply dwp_invert_imprecise in dwp; subst.
    ...
    + now refine (eval_ctx₀ phole (eval_beta _) I).
...
Proof.
  ...
  - crushTyping.
    ...
    + (* eapply termrel_size_left. *)
      ...
      eapply (termrel_ectx' vr); F.inferContext; E.inferContext;
        cbn; eauto using inject_value.
...
Proof.
  ...
  assert (vτ₂ : ValidPTy (embed τ₂)) by eauto using validPTy_embed.
  destruct (valrel_ptprod_inversion vτ₁ vτ₂ vr) as (vs1 & vs2 & vu1 & vu2 & -> & -> & ot1 & ot2 & vr').
  ...
  -  assert (fw : w < S w) by lia.
     ...
     eapply (termrelnd₀_ectx' inj₁); F.inferContext; E.inferContext; [|now cbn].
     intros vs3 vu3 vr3.
...
Proof.
  ...
  destruct (valrel_ptsum_inversion vτ₁ vτ₂ vr) as (vs' & vu' & [(? & ? & ot' & vrs)|(? & ? & ot' & vrs) ]);
    (* specialize (vrs w fw); *)
    subst; cbn in vvs.
  - eapply termrelnd₀_antired_left.
    { (* beta-reduce *)
...
Proof.
  ...
  - fold compfi_ty in *.
    ...
    eapply (termrel_ectx' trb); F.inferContext; E.inferContext; eauto using extract_value.
  ...
  - fold compfi_ty in div.
    ...
    replace τ₁ with (repEmul (embed τ₁)) at 2 by eapply repEmul_embed_leftinv.
    rewrite ?compiler_is_fxToIs_embed in *.
    refine (valrel_lambda _ _ _ _ _ _); eauto using validPTy_embed with tyeq tyvalid.
  ...
  - destruct (valrel_implies_Value vr1) as (vvs1 & _).
    ...
    * clear sz.
      ...
      { split.
        ...
        refine (OfTypeStlcEqui_fxToIs _ otu11);
      ...
      { split.
        ...
        rewrite repEmul_embed_leftinv.
        eauto using extractT with typing uval_typing.
        refine (OfTypeStlcEqui_fxToIs _ otu12);
        cbn; now apply compiler_is_fxToIs_embed.
      }
      { intros w' fw.
        ...
        eapply E.value_evalStar in eu3; now subst.
      }
      { intros w' fw.
        destruct (vr1' w' fw) as (vr11 & vr12).
        destruct w; [exfalso;lia|].
    ...
    * specialize (sz eq_refl).
      ...
      eapply (termrelnd₀_ectx' extr₁); [F.inferContext|E.inferContext|idtac|now cbn|now cbn].
    ...
    + clear extr2.
      ...
      * clear szvs.
        ...
        { destruct (OfType_inversion_pEmulDV ot) as (_ & _ & tyvs' & tyvu).
          ...
          inversion H1; subst.
          eapply E.invert_ty_inl in tyvu; eauto with tyvalid.
          destruct (tyvu) as (τ₁' & τ₂' & vτ₁₂ & vτ₂' & tvu & (tyeq₁ & tyeq₂)%tyeq_invert_tsum).
          split; split;
        ...
        cbn in vvu.
        now destruct (E.value_evalStar vvu eu').
      * specialize (szvs eq_refl).
        ...
        assert (nlen : n <= n) by lia.
        eapply dwp_invert_S in dwp.
        assert (szvs' : E.size vu <= w) by lia.
        specialize (extr1 n w vs' vu nlen dwp (fun _ => szvs') vr).
        refine (termrelnd₀_ectx (Cs := F.pinl F.phole) (Cu := E.pinl E.phole) I I extr1 _).
        intros vs2 vu2 vr2.
        eapply valrel_termrelnd₀.
        eapply valrel_inl'; eauto using validPTy_embed.
    + clear extr1.
      ...
      * clear szvs.
        ...
        { destruct (OfType_inversion_pEmulDV ot) as (_ & _ & tyvs' & tyvu).
          ...
          eapply E.invert_ty_inr in tyvu; eauto using validPTy_embed with tyvalid.
          destruct tyvu as (τ₁' & τ₂' & vτ₁₂ & vτ₂' & tvu & (tyeq₁ & tyeq₂)%tyeq_invert_tsum).
          split; split;
            try rewrite <-compiler_is_fxToIs_embed;
            eauto.
      ...
      * specialize (szvs eq_refl).
        ...
        refine (termrelnd₀_ectx (Cs := F.pinr F.phole) (Cu := E.pinr E.phole) I I extr2 _).
        intros vs2 vu2 vr2.
        eapply valrel_termrelnd₀.
...
Proof.
  ...
  - intros w wm γs γu envrel.
    ...
    eapply (termrel_ectx' lr); F.inferContext; E.inferContext;
