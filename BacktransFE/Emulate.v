...
Fixpoint emulate (n : nat) (t : TmA) : F.TmA :=
  match t with
    ea_var x => F.a_var x
  | ea_abs τ₁ τ₂ tb  => inArrDwnA τ₁ τ₂ n (F.a_abs (UValFE n τ₁) (UValFE n τ₂) (emulate n tb))
  | ea_app τ₁ τ₂ t₁ t₂ => uvalAppA n (emulate n t₁) (emulate n t₂) τ₁ τ₂
  | ea_unit => inUnitDwnA n F.a_unit
  | ea_true => inBoolDwnA n F.a_true
  | ea_false => inBoolDwnA n F.a_false
  | ea_ite τ t₁ t₂ t₃ => F.a_ite (UValFE n τ) (caseBoolUpA n (emulate n t₁)) (emulate n t₂) (emulate n t₃)
  | ea_pair τ₁ τ₂ t₁ t₂ => inProdDwnA τ₁ τ₂ n (F.a_pair (UValFE n τ₁) (UValFE n τ₂) (emulate n t₁) (emulate n t₂))
  | ea_proj₁ τ₁ τ₂ t => F.a_proj₁ (UValFE n τ₁) (UValFE n τ₂) (caseProdUpA n (emulate n t) τ₁ τ₂)
  | ea_proj₂ τ₁ τ₂ t => F.a_proj₂ (UValFE n τ₁) (UValFE n τ₂) (caseProdUpA n (emulate n t) τ₁ τ₂)
  | ea_inl τ₁ τ₂ t => inSumDwnA τ₁ τ₂ n (F.a_inl (UValFE n τ₁) (UValFE n τ₂) (emulate n t))
  | ea_inr τ₁ τ₂ t => inSumDwnA τ₁ τ₂ n (F.a_inr (UValFE n τ₁) (UValFE n τ₂) (emulate n t))
  | ea_caseof τ₁ τ₂ τ t₁ t₂ t₃ => F.a_caseof (UValFE n τ₁) (UValFE n τ₂) (UValFE n τ) (caseSumUpA n (emulate n t₁) τ₁ τ₂) (emulate n t₂) (emulate n t₃)
  | ea_seq τ t₁ t₂ => F.a_seq (UValFE n τ) (caseUnitUpA n (emulate n t₁)) (emulate n t₂)
...
Fixpoint emulate_pctx (n : nat) (C : PCtxA) : F.PCtxA :=
  match C with
    | ea_phole => F.a_phole
    | ea_pabs τ₁ τ₂ C => F.pctxA_cat (F.a_pabs (UValFE n τ₁) (UValFE n τ₂) (emulate_pctx n C)) (inArrDwn_pctxA τ₁ τ₂ n)
    | ea_papp₁ τ₁ τ₂ C t₂ => F.pctxA_cat (emulate_pctx n C) (uvalApp_pctxA₁ n (emulate n t₂) τ₁ τ₂)
    | ea_papp₂ τ₁ τ₂ t₁ C => F.pctxA_cat (emulate_pctx n C) (uvalApp_pctxA₂ n (emulate n t₁) τ₁ τ₂)
    | ea_pite₁ τ C₁ t₂ t₃ => F.a_pite₁ (UValFE n τ) (F.pctxA_cat (emulate_pctx n C₁) (caseBoolUp_pctxA n)) (emulate n t₂) (emulate n t₃)
    | ea_pite₂ τ t₁ C₂ t₃ => F.a_pite₂ (UValFE n τ) (caseBoolUpA n (emulate n t₁)) (emulate_pctx n C₂) (emulate n t₃)
    | ea_pite₃ τ t₁ t₂ C₃ => F.a_pite₃ (UValFE n τ) (caseBoolUpA n (emulate n t₁)) (emulate n t₂) (emulate_pctx n C₃)
    | ea_ppair₁ τ₁ τ₂ C₁ t₂ => F.pctxA_cat (F.a_ppair₁ (UValFE n τ₁) (UValFE n τ₂) (emulate_pctx n C₁) (emulate n t₂)) (inProdDwn_pctxA τ₁ τ₂ n)
    | ea_ppair₂ τ₁ τ₂ t₁ C₂ => F.pctxA_cat (F.a_ppair₂ (UValFE n τ₁) (UValFE n τ₂) (emulate n t₁) (emulate_pctx n C₂)) (inProdDwn_pctxA τ₁ τ₂  n)
    | ea_pproj₁ τ₁ τ₂ C => F.a_pproj₁ (UValFE n τ₁) (UValFE n τ₂) (F.pctxA_cat (emulate_pctx n C) (caseProdUp_pctxA n τ₁ τ₂))
    | ea_pproj₂ τ₁ τ₂ C => F.a_pproj₂ (UValFE n τ₁) (UValFE n τ₂) (F.pctxA_cat (emulate_pctx n C) (caseProdUp_pctxA n τ₁ τ₂))
    | ea_pinl τ₁ τ₂ C => F.pctxA_cat (F.a_pinl (UValFE n τ₁) (UValFE n τ₂) (emulate_pctx n C)) (inSumDwn_pctxA τ₁ τ₂ n)
    | ea_pinr τ₁ τ₂ C => F.pctxA_cat (F.a_pinr (UValFE n τ₁) (UValFE n τ₂) (emulate_pctx n C)) (inSumDwn_pctxA τ₁ τ₂ n)
    | ea_pcaseof₁ τ₁ τ₂ τ C₁ t₂ t₃ => F.a_pcaseof₁ (UValFE n τ₁) (UValFE n τ₂) (UValFE n τ) (F.pctxA_cat (emulate_pctx n C₁) (caseSumUp_pctxA n τ₁ τ₂)) (emulate n t₂) (emulate n t₃)
    | ea_pcaseof₂ τ₁ τ₂ τ t₁ C₂ t₃ => F.a_pcaseof₂ (UValFE n τ₁) (UValFE n τ₂) (UValFE n τ) (caseSumUpA n (emulate n t₁) τ₁ τ₂) (emulate_pctx n C₂) (emulate n t₃)
    | ea_pcaseof₃ τ₁ τ₂ τ t₁ t₂ C₃ => F.a_pcaseof₃ (UValFE n τ₁) (UValFE n τ₂) (UValFE n τ) (caseSumUpA n (emulate n t₁) τ₁ τ₂) (emulate n t₂) (emulate_pctx n C₃)
    | ea_pseq₁ τ C₁ t₂ => F.a_pseq₁ (UValFE n τ) (F.pctxA_cat (emulate_pctx n C₁) (caseUnitUp_pctxA n)) (emulate n t₂)
    | ea_pseq₂ τ t₁ C₂ => F.a_pseq₂ (UValFE n τ) (caseUnitUpA n (emulate n t₁)) (emulate_pctx n C₂)
    | ea_pcoerce τ C => emulate_pctx n C
  end.

Fixpoint toUVals n (Γ : E.Env) : F.Env :=
  match Γ with
    E.empty => F.empty
  | Γ r▻ τ => toUVals n Γ ▻ UValFE n τ
  end.

Lemma toUVals_entry {n Γ i τ} :
  ⟪  i : τ r∈ Γ  ⟫ → ⟪ i : UValFE n τ ∈ toUVals n Γ ⟫.
Proof.
...
Lemma emulateT {n t Γ τ} :
  ...
  ⟪  Γ ea⊢ t : τ  ⟫ ->
  ⟪  toUVals n Γ a⊢ emulate n t : UValFE n τ  ⟫.
Proof.
  intros vΓ vτ tyτ.
  revert vΓ vτ.
  induction tyτ;
  ...
  - eapply uvalAppA_T.
    ...
    eapply (IHtyτ1 vΓ).
    refine (typed_terms_are_valid _ _ vΓ (eraseAnnotT tyτ1)).
    eapply (IHtyτ2 vΓ).
    refine (typed_terms_are_valid _ _ _ (eraseAnnotT tyτ2));
      eauto with tyvalid.
  - constructor.
    eapply caseProdUpA_T; try assumption.
    assert (vτ₁₂ := typed_terms_are_valid _ _ vΓ (eraseAnnotT tyτ)).
    now eapply ValidTy_invert_prod in vτ₁₂.
    eapply (IHtyτ vΓ).
    refine (typed_terms_are_valid _ _ vΓ (eraseAnnotT tyτ)).
  - constructor.
    eapply caseProdUpA_T; try assumption.
    assert (vτ₁₂ := typed_terms_are_valid _ _ vΓ (eraseAnnotT tyτ)).
    now eapply ValidTy_invert_prod in vτ₁₂.
    ...
    refine (typed_terms_are_valid _ _ vΓ (eraseAnnotT tyτ)).
  - constructor.
    eapply caseSumUpA_T; try assumption.
    eapply (IHtyτ1 vΓ).
    refine (typed_terms_are_valid _ _ vΓ (eraseAnnotT tyτ1)).
    eapply IHtyτ2; crushValidTy.
    eapply IHtyτ3; crushValidTy.
...
Lemma emulate_pctx_T {n C Γₒ τₒ Γ τ} :
  ValidEnv Γ -> ValidTy τ ->
  ⟪  ea⊢ C : Γₒ , τₒ → Γ , τ  ⟫ ->
  ⟪  a⊢ emulate_pctx n C : toUVals n Γₒ , UValFE n τₒ → toUVals n Γ , UValFE n τ  ⟫.
Proof.
  intros vΓ vτ.
  induction 1; cbn;
    crushTypingA;
    ...
    try change (toUVals n ?Γ ▻ UValFE n ?τ) with (toUVals n (Γ r▻ τ));
    eauto using emulateT, inArrDwn_pctx_T with typing uval_typing tyvalid.
    (* repeat match goal with *)
    (* | H: ⟪ _ ea⊢ _ : ?τ ⟫ |- ValidTy _ => assert (ValidTy τ); [refine (typed_terms_are_valid _ _ _ (eraseAnnotT H))|] *)
    (* end; *)
    (* eauto using emulateT, inArrDwn_pctx_T with typing uval_typing tyvalid. *)
  - assert (vτ₁ := typed_terms_are_valid _ _ vΓ (eraseAnnotT H1)).
    eapply IHPCtxTypingAnnot; crushValidTy.
  - assert (vτ₁ := typed_terms_are_valid _ _ vΓ (eraseAnnotT H1)).
    eapply uvalApp_pctxA₁_T; try assumption.
    eapply emulateT; try assumption.
  - rewrite <-(UVal_eq H1 H2 H).
    eapply IHPCtxTypingAnnot; try assumption.
Qed.
...
Lemma compat_emul_unit {Γ d n p m} :
  ...
  ⟪ Γ ⊩ inUnitDwn n F.unit ⟦ d , m ⟧ E.unit : pEmulDV n p tunit ⟫.
Proof.
  intros dwp.
  split; [|split].
...
Lemma compat_emul_true {Γ d n p m} :
  dir_world_prec n m d p →
  ⟪ Γ ⊩ inBoolDwn n F.true ⟦ d , m ⟧ E.true : pEmulDV n p tbool ⟫.
Proof.
  intros dwp.
  split; [|split].
...
Lemma compat_emul_false {Γ d n p m} :
  dir_world_prec n m d p →
  ⟪ Γ ⊩ inBoolDwn n F.false ⟦ d , m ⟧ E.false : pEmulDV n p tbool ⟫.
Proof.
  intros dwp.
  split; [|split].
  - eauto using inBoolDwn_T with typing uval_typing.
...
Lemma compat_emul_abs {n m d p Γ τ₁ τ₂ ts tu} :
  ...
  ⟪ Γ ⊩ inArrDwn τ₁ τ₂ n (F.abs (UValFE n τ₁) ts) ⟦ d , m ⟧ E.abs τ₁ tu : pEmulDV n p (E.tarr τ₁ τ₂) ⟫.
Proof.
  ...
  - intros w wn γs γu envrel.
    ...
    assert (OfType (ptarr (pEmulDV n p τ₁) (pEmulDV n p τ₂))
                   (F.abs (repEmul (pEmulDV n p τ₁))
                          (ts [γs↑])) (E.abs τ₁ (tu [γu↑]))) by
      (pose proof (envrel_implies_WtSub envrel);
    ...
    change (UValFE n τ₁) with (repEmul (pEmulDV n p τ₁)).

    refine (valrel_lambda _ _ _ _ _ _); crush; [eapply tyeq_refl|].
...
(*   dir_world_prec n m d p → *)
(*   ⟪ ⊩ F.pctx_cat (F.pabs (UVal n) F.phole) (inArrDwn_pctx n) ⟦ d , m ⟧ E.pabs E.phole : Γ p▻ pEmulDV n p , pEmulDV n p → Γ , pEmulDV n p ⟫. *)
(* Proof. *)
(*   intros dwp. *)
(*   split. *)
...
Lemma compat_emul_pair {n m d p Γ τ₁ τ₂ ts₁ tu₁ ts₂ tu₂} :
  ...
  ⟪ Γ ⊩ ts₂ ⟦ d , m ⟧ tu₂ : pEmulDV n p τ₂ ⟫ →
  ⟪ Γ ⊩ inProdDwn τ₁ τ₂ n (F.pair ts₁ ts₂) ⟦ d , m ⟧ E.pair tu₁ tu₂ : pEmulDV n p (E.tprod τ₁ τ₂)⟫.
Proof.
  ...
  - intros w wm γs γu envrel.
    ...
    enough (termrel d w (ptprod (pEmulDV n p τ₁) (pEmulDV n p τ₂)) (F.pair ts₁ ts₂)[γs] (E.pair tu₁ tu₂)[γu]) as trp.
    + eapply (termrel_ectx' trp); F.inferContext; E.inferContext; simpl; crush.
      intros w' futw vs vu vr.
      eapply termrel₀_in_termrel.
      eapply termrel₀_inProdDwn;
        eauto using dwp_mono with arith.
    + eapply termrel_pair;
        fold F.apTm E.apTm; crush.
...
Lemma termrel_emul_caseof {n w d p τ₁ τ₂ τ ts₁ tu₁ ts₂ tu₂ ts₃ tu₃} :
  ...
  (forall w' vs₃ vu₃, w' ≤ w →
                      valrel d w' (pEmulDV n p τ₂) vs₃ vu₃ →
                      termrel d w' (pEmulDV n p τ) (ts₃ [beta1 vs₃]) (tu₃ [beta1 vu₃])) →
  termrel d w (pEmulDV n p τ) (F.caseof (caseSumUp n ts₁ τ₁ τ₂) ts₂ ts₃) (E.caseof tu₁ tu₂ tu₃).
Proof.
  intros vτ₁ vτ₂ vτ dwp tr₁ tr₂ tr₃.
  unfold caseSumUp.
  (* evaluate ts₁ and ts₂ *)
  eapply (termrel_ectx' tr₁); F.inferContext; E.inferContext; crush;
  ...
  (* evaluate upgrade *)
  assert (trupg : termrel d w' (pEmulDV (n + 1) p (τ₁ r⊎ τ₂)) (F.app (upgrade n 1 (τ₁ r⊎ τ₂)) vs) vu) by
    (eauto using upgrade_works', dwp_mono with tyvalid).
  replace (n + 1) with (S n) in trupg by lia.
  eapply (termrel_ectx' trupg); F.inferContext; E.inferContext;
    crush.
...
Lemma compat_emul_caseof {n m d p τ₁ τ₂ τ Γ ts₁ tu₁ ts₂ tu₂ ts₃ tu₃} :
  ...
  ⟪ Γ p▻ pEmulDV n p τ₁ ⊩ ts₂ ⟦ d , m ⟧ tu₂ : pEmulDV n p τ ⟫ →
  ⟪ Γ p▻ pEmulDV n p τ₂ ⊩ ts₃ ⟦ d , m ⟧ tu₃ : pEmulDV n p τ ⟫ →
  ⟪ Γ ⊩ F.caseof (caseSumUp n ts₁ τ₁ τ₂) ts₂ ts₃ ⟦ d , m ⟧ E.caseof tu₁ tu₂ tu₃ : pEmulDV n p τ ⟫.
Proof.
...
Lemma termrel_emul_ite {n w d p τ ts₁ tu₁ ts₂ tu₂ ts₃ tu₃} :
  ...
  (forall w', w' ≤ w → termrel d w' (pEmulDV n p τ) ts₃ tu₃) →
  termrel d w (pEmulDV n p τ) (F.ite (caseBoolUp n ts₁) ts₂ ts₃) (E.ite tu₁ tu₂ tu₃).
Proof.
  ...
  replace (n + 1) with (S n) in trupg by lia.
  eapply (termrel_ectx' trupg); F.inferContext; E.inferContext; crush; [|now cbn].
...
Lemma compat_emul_ite {n m d p Γ τ ts₁ tu₁ ts₂ tu₂ ts₃ tu₃} :
  ...
  ⟪ Γ ⊩ ts₂ ⟦ d , m ⟧ tu₂ : pEmulDV n p τ ⟫ →
  ⟪ Γ ⊩ ts₃ ⟦ d , m ⟧ tu₃ : pEmulDV n p τ ⟫ →
  ⟪ Γ ⊩ F.ite (caseBoolUp n ts₁) ts₂ ts₃ ⟦ d , m ⟧ E.ite tu₁ tu₂ tu₃ : pEmulDV n p τ ⟫.
Proof.
...
Lemma termrel_emul_seq {n w d p τ ts₁ tu₁ ts₂ tu₂} :
  ...
  (forall w', w' ≤ w → termrel d w' (pEmulDV n p τ) ts₂ tu₂) →
  termrel d w (pEmulDV n p τ) (F.seq (caseUnitUp n ts₁) ts₂) (E.seq tu₁ tu₂).
Proof.
  intros dwp tr₁ tr₂.
  unfold caseUnitUp.

  (* evaluate ts₁ and tu₁ *)
  eapply (termrel_ectx' tr₁); F.inferContext; E.inferContext; cbn; eauto using caseUnitUp_pctx_ectx.
...
Lemma compat_emul_seq {n m d p Γ τ ts₁ tu₁ ts₂ tu₂} :
  ...
  ⟪ Γ ⊩ ts₂ ⟦ d , m ⟧ tu₂ : pEmulDV n p τ ⟫ →
  ⟪ Γ ⊩ F.seq (caseUnitUp n ts₁) ts₂ ⟦ d , m ⟧ E.seq tu₁ tu₂ : pEmulDV n p τ ⟫.
Proof.
  intros vΓ dwp (? & ? & tr₁) (? & ? & tr₂).
  split; [|split];
...
Lemma termrel_emul_inl {n w d p τ₁ τ₂ ts tu} :
  ...
  termrel d w (pEmulDV n p (τ₁ r⊎ τ₂)) (inSumDwn τ₁ τ₂ n (F.inl ts)) (E.inl tu).
Proof.
  intros vτ₁ vτ₂ dwp tr.
  unfold inSumDwn.
  eapply (termrel_ectx' tr); F.inferContext; E.inferContext; crush;
  ...
  intros w' fw vs vu vr.
  eapply termrel₀_in_termrel.
  eapply termrel₀_inSumDwn; simpl; eauto using valrel_inl, dwp_mono with tyvalid.
Qed.
...
Lemma compat_emul_inl {n m d p τ₁ τ₂ Γ ts tu} :
  ...
  dir_world_prec n m d p →
  ⟪ Γ ⊩ ts ⟦ d , m ⟧ tu : pEmulDV n p τ₁ ⟫ →
  ⟪ Γ ⊩ inSumDwn τ₁ τ₂ n (F.inl ts) ⟦ d , m ⟧ E.inl tu : pEmulDV n p (τ₁ r⊎ τ₂) ⟫.
Proof.
...
Lemma termrel_emul_inr {n w d p τ₁ τ₂ ts tu} :
  ...
  dir_world_prec n w d p →
  termrel d w (pEmulDV n p τ₂) ts tu →
  termrel d w (pEmulDV n p (τ₁ r⊎ τ₂)) (inSumDwn τ₁ τ₂ n (F.inr ts)) (E.inr tu).
Proof.
...
Lemma compat_emul_inr {n m d p τ₁ τ₂ Γ ts tu} :
  ...
  dir_world_prec n m d p →
  ⟪ Γ ⊩ ts ⟦ d , m ⟧ tu : pEmulDV n p τ₂ ⟫ →
  ⟪ Γ ⊩ inSumDwn τ₁ τ₂ n (F.inr ts) ⟦ d , m ⟧ E.inr tu : pEmulDV n p (τ₁ r⊎ τ₂)⟫.
Proof.
  intros vτ₁ vτ₂ dwp lr.
  destruct lr as (? & ? & tr).
  split; [|split].
...
Lemma termrel_emul_proj₁ {n w d p τ₁ τ₂ ts tu} :
  ...
  termrel d w (pEmulDV n p τ₁) (F.proj₁ (caseProdUp n ts τ₁ τ₂)) (E.proj₁ tu).
Proof.
  intros vτ₁ vτ₂ dwp tr.
  unfold caseProdUp.
  ...
  (* execute the upgrade *)
  assert (trupg : termrel d w' (pEmulDV (n + 1) p (E.tprod τ₁ τ₂)) (F.app (upgrade n 1 (E.tprod τ₁ τ₂)) vs) vu)
    by eauto using upgrade_works', dwp_mono with tyvalid.
  replace (n + 1) with (S n) in trupg by lia.
  eapply (termrel_ectx' trupg); F.inferContext; E.inferContext; crush; [|now cbn].
  ...
  - eapply termrel_antired_star_left.
    ...
    refine (termrel_proj₁ _ _ vr''); cbn; eauto with tyvalid.
...
Lemma compat_emul_proj₁ {n m d p Γ τ₁ τ₂ ts tu} :
  ...
  ⟪ Γ ⊩ F.proj₁ (caseProdUp n ts τ₁ τ₂) ⟦ d , m ⟧ E.proj₁ tu : pEmulDV n p τ₁ ⟫.
Proof.
  intros vτ₁ vτ₂ dwp lr.
  destruct lr as (? & ? & tr).
  split; [|split].
