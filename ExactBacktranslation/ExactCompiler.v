...
Theorem exact_equi_annot_roundtrip {t A} :
  ...
  StlcEqui.SpecEquivalent.PCtxEquivalent empty
    (CIEC.compie (IAC.eraseAnnot (compile_equi_annot A t)))
    (EAC.eraseAnnot t) A.
Proof.
  intros Ht. rewrite erase_compile_equi_annot.
  ...
  StlcEqui.SpecEquivalent.PCtxEquivalent empty
    (CIEC.compie
      (StlcIso.SpecSyntax.pctx_app
        (IAC.eraseAnnot (compile_equi_annot A t))
        (IAC.eraseAnnot_pctx (compile_equi_context_annot R C))))
    (StlcEqui.SpecSyntax.pctx_app (EAC.eraseAnnot t)
      (EAC.eraseAnnot_pctx C)) R.
Proof.
  intros Ht HC.
  rewrite erase_compile_equi_annot, erase_compile_equi_context_annot.
...
Theorem exact_equi_annot_full_abstraction {t1 t2 A} :
  ...
  (StlcEqui.SpecEquivalent.PCtxEquivalent empty
   ...
   StlcIso.SpecEquivalent.PCtxEquivalent empty
      (IAC.eraseAnnot (compile_equi_annot A t1))
      (IAC.eraseAnnot (compile_equi_annot A t2)) A).
Proof.
  intros Ht1 Ht2. rewrite !erase_compile_equi_annot.
